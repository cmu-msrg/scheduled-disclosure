...
{
        int err = 0;
        inpoutx64_dll = LoadLibrary("inpoutx64.dll");
        if (inpoutx64_dll == NULL) {
                return -ENODATA;
        }


        for (size_t i = 0; i < sizeof(symbol_list2) / sizeof(symbol_list2[0]); i++) {
                struct symbol *sym = &symbol_list2[i];
